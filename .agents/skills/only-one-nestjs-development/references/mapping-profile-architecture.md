    ...
    get profile(): MappingProfile {
        return (mapper) => {
            createMap(
                ...
                forMember(
                    (dest) => dest.owner,
                    mapWith(OwnerDto, OwnerEntity, (src) => (src.owner && wrap(src.owner).isInitialized() ? src.owner : null)),
                ),
                forMember(
                    (dest) => dest.items,
                    mapWith(ItemDto, ItemEntity, (src) => (src.items?.isInitialized() ? src.items.getItems() : [])),
                ),
            );
