...
| **`ui-ux-pro-max`** _(via `ui-ux-pro-max-cli`)_     | User interface creation, redesign, component styling, or UI/UX audit                             | Enforce design intelligence (color palette, typography, spacing rhythm, WCAG contrast/accessibility, UX heuristics) in Section 2 & 3.                         |
...
- _Note_: Using `<YYYYMMDD-HHmmss>` ensures tasks are always sorted chronologically at the bottom.
