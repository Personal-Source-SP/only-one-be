...
export class CloudDataProviderController extends BaseController<CloudDataProviderEntity, CloudDataProviderDto> {
    ...
    @Version('1')
    @HttpCode(HttpStatus.OK)
    @Post('upload/:cloudDataProviderId')
    @ApiFile({ description: 'File to upload' })
    @BaseApiOkResponse(UploadFileResponse)
    async uploadFile(
        @UploadedFile() file: Express.Multer.File,
        @Param('cloudDataProviderId', new ParseUUIDPipe()) cloudDataProviderId: string,
        @Body() request: CloudDataUploadFileRequest,
    ): Promise<UploadFileResponse> {
        if (!file) throw new BadRequestException('No file uploaded');

        const response = await this.cloudDataProviderService.uploadFile(file, cloudDataProviderId, request);
        return response;
    }
