---
id: 20260916-110600-network-device-discovery-and-probing-suite
...
- **Vấn đề (Problem)**:
  ...
  - Ban đầu bản tin ONVIF SOAP XML bị sai lệch namespace (`<dn:Probe>` thuộc WSDL namespace thay vì WS-Discovery schema `http://schemas.xmlsoap.org/ws/2005/04/discovery`), bộ lọc `Types` bị gán cứng `dn:NetworkVideoTransmitter`, và chỉ gửi multicast `239.255.255.250` dẫn đến camera/thiết bị mạng drop gói tin.
  - Sau khi quét ra IP, `DeviceAggregatorService` bị liên kết cứng (hard-coupled) với các probe service rời rạc, chưa chuẩn hóa thành 3 Hướng Tiếp Cận (3 Approaches), thiếu khả năng sandbox test độc lập.
  - `ProtocolAuthApproachService` từng gọi trực tiếp `axios.get(...)` thay vì qua `BaseHttpService`, thiếu cấu hình timeouts và ports tập trung (magic strings/numbers), xử lý lỗi nuốt silent error và hardcode error messages thô không theo chuẩn `IAppError`.
- **Giá trị (Value)**:
  ...
  - Dịch vụ **`ProtocolAuthApproachService`** tự động xác thực tài khoản camera với từ điển mặc định/tùy chỉnh qua `BaseHttpService`, trích xuất RTSP stream URI, snapshot preview thumbnail và metadata thiết bị với error handling chuẩn `IAppError` (`TargetIpRequired`, `CameraAuthFailed`).
...
- **Enum `NetworkDeviceApproachEnum`**:
  ...
  - `PROTOCOL_AUTH`: Tầng Ứng dụng & Xác thực (Layer 7) - Thử danh sách credentials mẫu/tùy chỉnh qua `BaseHttpService`, kết nối ONVIF Device/Media management, trích xuất stream RTSP và snapshot thumbnail.
...
flowchart TD
    ...
    subgraph Approaches ["3 Strategy Approaches"]
        ...
        ProtoAuth["ProtocolAuthApproachService\n(ONVIF Auth + BaseHttpService + RTSP + Snapshot)"]
    end
...
- [onvif.constant.ts](file:///Users/kiem/Sources/PERSONAL/only-one-be/src/modules/network-device/constants/onvif.constant.ts): Bản tin WS-Discovery SOAP XML, camera credentials mặc định, timeout, và paths snapshot/auth.
- [network-device-error.ts](file:///Users/kiem/Sources/PERSONAL/only-one-be/src/modules/network-device/constants/network-device-error.ts): Định nghĩa mã lỗi chuẩn `IAppError` bao gồm `TargetIpRequired` và `CameraAuthFailed`.
...
- [oui-lookup.helper.ts](file:///Users/kiem/Sources/PERSONAL/only-one-be/src/modules/network-device/helpers/oui-lookup.helper.ts): Tra cứu OUI MAC để xác định Vendor phần cứng.
...
- [protocol-auth-approach.service.ts](file:///Users/kiem/Sources/PERSONAL/only-one-be/src/modules/network-device/services/network-device-approach/protocol-auth-approach.service.ts): Triển khai Hướng Tiếp Cận 3 (ONVIF & Camera Auth tích hợp `BaseHttpService`).
- [network-device-approach.provider.ts](file:///Users/kiem/Sources/PERSONAL/only-one-be/src/modules/network-device/services/network-device-approach/network-device-approach.provider.ts): Provider factory đăng ký các approach services.
...
- [network-device.controller.ts](file:///Users/kiem/Sources/PERSONAL/only-one-be/src/modules/network-device/controllers/network-device.controller.ts): REST API endpoints cho approach sandbox test.
- [device-aggregator.controller.ts](file:///Users/kiem/Sources/PERSONAL/only-one-be/src/modules/network-device/controllers/device-aggregator.controller.ts): REST API endpoints cho trigger scan và scan status.
...
- **Unit Tests**:
  ...
  - Module `network-device` test suite: 39/39 tests passed across 20 suites $\rightarrow$ `PASS (100%)`.
- **TypeScript Strict Build**: `npx tsc -p tsconfig.build.json` $\rightarrow$ `PASS (0 errors)`.
